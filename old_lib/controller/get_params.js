...
module.exports = function (controller, which, callback) {
    //  console.log('get_params: req state = %s', util.inspect(req_state, null, 0));

    var registry = {};

    function _done() {
        console.log('controller get_params done with registry %s', util.inspect(registry))
    ...
    which.forEach(function (def) {
        ...
        if (!error) {
            controller.get_param(controller, what_what, _what_done, absent);
        }
