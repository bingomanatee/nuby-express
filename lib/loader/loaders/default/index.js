...
module.exports = {
    ...
    _load_action:function (controller, action_path, done_cb) {
        var full_action_path = action_path + '/index.js';
        var config = {};
        if (path.existsSync(full_action_path)) {
            config = require(full_action_path);
        } else {
            full_action_path = action_path + '/action.js';
            if (path.existsSync(full_action_path)) {
                config = require(full_action_path);
            } else {
                full_action_path = action_path + '/' + path.basename(action_path);
                if (path.existsSync(full_action_path)) {
                    config = require(full_action_path);
                }
            }
        ...
        _debug('loading action %s with controller %s', action_path, util.inspect(controller, null, 0));
        ...
        _debug('loading handler for %s (%s)', route, action.method);
