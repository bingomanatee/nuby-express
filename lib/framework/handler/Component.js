...
module.exports = function(){
    return new Path_Handler({
        ...
        execute:function (match_path, callback, target, match) {
            if (!target.load_component){
                console.log('Framework Componeent Handler: no load_component in target %s', util.inspect(target));
                throw new Error('Framework Component handler: bad target');
            }
