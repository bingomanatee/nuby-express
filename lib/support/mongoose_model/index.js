...
var _va_index_re = /duplicate key .* dup key: (.*)/;
var _va_validator_re = /Validator \\"([^\\"]+)\\" failed for path ([\w.]+)/;
MongooseModel.prototype = {
    ...
    count:function (cond, callback) {
        this.model.count(cond, callback);
    },
    ...
    validation_errors:function (err) {
        ...
        for (var field in err.errors) {
            ...
            list.push(util.format("mongoose validation errror for '%s': %s", error.path, error.type));
        }
        return list.join(',');
    }
