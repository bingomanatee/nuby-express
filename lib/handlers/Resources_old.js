        ...
        execute:function (match_path, callback, target, match) {
            ...
            fs.readdir(match_path, function (err, files) {
                ...
                files.forEach(function (type) {
                    ...
                    if (_.contains(known_types.concat(_.map(known_types, function(t){ return t + 's'})), type)) {
                        type = type.replace(/s$/, '');
                        // is a resource directory or file; we trust the owner to have an appropriate handlers for resource
                        ...
                        console.log('not a type: %s, loading as resource', type);
