        ...
        start_load:function (callback, root, frame, context) {
          ...
          //  file_gate.debug = true;
            ...
            fs.readdir(root, function (err, files) {
                ...
                file_gate.start();
/*
                self._read_queue.push(files, function () {
                    logger.log(util.format('READ A FILE IN DIR %s(%s); %s pending tasks',
                        self.id(), self.path, self._pending_tasks.length));
                });

                self._read_queue.drain = function () {
                    console.log('READ __ALL FILES__ IN DIR %s(%s); %s pending tasks',
                        self.id(), self.path, self._pending_tasks.length);
                    logger.log(util.format('READ __ALL FILES__ IN DIR %s(%s); %s pending tasks',
                        self.id(), self.path, self._pending_tasks.length));
                    self._pending_task_gate.start();
        ...
        check_file_count:function () {
            ...
            if (this._all_files_read && (this._file_count == 0)) {
                ...
                if (this._pending_tasks.length > 0) {
                    ...
                    /*    logger.log({msg:'pending tasks pushed', tasks:self._pt_string(), loader:this.id()});
                     this._pending_task_queue.push(this._pending_tasks, function (err){
                     if (err){
                     logger.error(err);
                     }
                     logger.log('finished pending task');
                     });*/
